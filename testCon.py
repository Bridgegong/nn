...
cond = threading.Condition() # 判断条件


class erkang(threading.Thread):

    def __init__(self, cond, diaosinan):
        threading.Thread.__init__(self, name=diaosinan)
        self.cond = cond

    def run(self):
        self.cond.acquire() # 获得锁
...
class ziwei(threading.Thread):
    def __init__(self, cond, diaosinv):
        threading.Thread.__init__(self, name=diaosinv)
        self.cond = cond
